...
def apply_sobel_convolution(image):
...
def doTests():
    ...
    for test in tests:
        ...
        for i, imfile in enumerate(files):
            ...
            if test == "testSobel":
                outs_np = testSobel(im, {})
                gx, gy, magnitude, binary_edge = outs_np
                plt.figure(figsize=(12, 6))
                plt.subplot(1, 2, 1)
                plt.imshow(gx, cmap='gray')
                plt.title('I1 Horizontal Edges (Convolution)')
                plt.subplot(1, 2, 2)
                plt.imshow(gy, cmap='gray')
                plt.title('I1 Vertical Edges (Convolution)')
                plt.show()
            elif test in ["testCanny", "testHough"]:
                params = {'sigma': 5}
